...
\label{a_MCTS_Expand}
    \Fn{\textbf{expand}($n_j, \hspace{1.5mm} prediction_{orig}$)}{
        ...
        \uIf{$score_j > 1$ \label{a_CoDy_Expand_l_cf}}{
            ...
            \If{$children_j = \varnothing$}{
                $selectable_j \gets 0$\;\label{a_CoDy_Expand_l_no_children}
            }
        }
    }
\end{algorithm}
...
\label{a_MCTS_Backpropagation}
    \Fn{\textbf{backpropagate}($n_j$)}{
        ...
        \If{No child in $children_j$ is selectable}{
            $selectable_j \gets 0$\; \label{a_CoDy_Backprop_score_not_selectable}
        }
    }
\end{algorithm}
}
