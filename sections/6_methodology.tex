...
\label{a_GreedyCF}
    ...
    $n_{root} \gets (\varnothing, prediction_{orig}, \varnothing, \varnothing)$\;

    $n_{best} \gets n_{root}$;

    \While{$s_{best}$ $\mathrm{does\ not\ include\ all\ candidate\ events}$
    $C(\mathcal{G}, \varepsilon_i, k, m_{max})$}{
        $children_{best} \gets$ set of $l$ child node with highest rank according to $\delta$, each child $n_{child}$ initialized with prediction $prediction_{child} = f(\mathcal{G}(t_i) \setminus s_{child}, \varepsilon_i)$\;
        $n_{best\_child} \gets \argmax_{n_j \in children_{best}} \Delta(prediction_{orig}, prediction_j)$\;
        \uIf{$\Delta(prediction_{best}, prediction_{best\_child}) > 0$}{
